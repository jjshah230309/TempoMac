...
enum MediaControl {
    ...
    private static func runningTarget() -> String? {
        ...
        let spotifyUp = running.contains("com.spotify.client")
        let musicUp   = running.contains("com.apple.Music")
...
final class AgentMenuBar: NSObject {
    ...
    func start(store: LibraryStore) {
        ...
        // plays), so the "today" total updates — and re-render on a timer so it
        // rolls over to a fresh day even if nothing else happens.
        store.objectWillChange
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in self?.updateContent() }
            .store(in: &bag)
        // 5s keeps the icon's progress hairline moving and rolls the "today"
        // total into a new day. The player is only queried while something is
        // actually playing, so an idle Mac does no work here.
...
enum BackgroundAgent {
    ...
    private static func install() throws {
        let plist: [String: Any] = [
            ...
            "ThrottleInterval": 1,       // don't sit in launchd's default 10s respawn throttle
            "ProcessType": "Interactive", // full priority so the menu bar item appears at once
        ]
